...
Router::Router(const Router &other) : _config(other._config),
	_path(other._path), _query(other._query), _method(other._method),
	_documentRoot(other._documentRoot), _absolutePath(other._absolutePath)
{
...
bool Router::build_FinalPath(std::string &path)
{
	...
	hasTrailingSlash = (path.size() > 0 && path[path.size() - 1] == '/');
...
Response Router::handle_GET(const Request &request, Location &location)
{
	...
	if (is_Directory(_absolutePath))
	{
		...
		else if (location.autoIndex)
		{
			...
			response.set_Header("Content-Type",
				get_MimeType(get_Extension(".html"), _config.mimeTypes.types));
			return response;
		}
		else
			return make_ErrorCode(403);
	...
	std::string MimeType = get_MimeType(get_Extension(_absolutePath),
			_config.mimeTypes.types);
...
Response Router::handle_POST(const Request &request, Location &location)
{
	...
	if (!_absolutePath.empty() && _absolutePath[_absolutePath.size()
		- 1] != '/')
		_absolutePath += '/';
...
Response Router::handle_Request(const Request &request)
{
	...
	if (!validate_Method(request.get_Method()))
		return make_ErrorCode(405);
	if (_config.config.client_max_body_size > 0
		&& request.get_Body().size() > _config.config.client_max_body_size)
		return make_ErrorCode(413);
